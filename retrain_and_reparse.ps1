...
#
# Phase 4 changes active in this script:
#   - parse_annotated_pgn.py  outputs history_rich (144-dim GRU) + 1663-dim algo features
#   - ingest_lichess_csv.py   outputs history_rich=[] + 1663-dim algo features
#   - Silver labels added for deterministic concepts on unlabeled positions
#   - Training uses --phase4 flag → COMBINED_SIZE_V4=3048, MOVE_SIZE_V4=144 (GRU step; static move=128)
#
...
#SKIP: training_raw.jsonl already written (9.1 GB) — crash was during training, not parsing
...
# #SKIP: already appended to training_raw.jsonl in the same run
...
# -- 3b. Build algo feature cache (strips 1663-float arrays from JSONL → binary) --
